...
@available(*, deprecated)
public final class Game <Question, Answer, R: Router> {
    let flow: Any
...
public func startGame<Question, Answer: Equatable, R: Router>(questions: [Question], router: R, correctAnswers: [Question: Answer]) -> Game<Question, Answer, R> where R.Question == Question, R.Answer == Answer {
    let flow = Flow(questions: questions, delegate: QuizDelegateToRouterAdapter(router, correctAnswers), scoring: {
...
@available(*, deprecated)
private final class QuizDelegateToRouterAdapter<R: Router>: QuizDelegate where R.Answer: Equatable {
    ...
    func didCompleteQuiz(withAnswers answers: [(question: R.Question, answer: R.Answer)]) {
        ...
        let result = Result(answer: answersDictionary, score: score)
